...
# 1. 랜덤 포레스트 모델 생성 
# 형식) randomForest(y ~ x, data, ntree, mtry) # 만드는 데이터셋의 수 ntree : 500개가 기본
model = randomForest(Species~., data=iris)  
model
# No. of variables tried at each split: 2 : 노드를 나누는 가지의 수 최대 4개
...
# 2. 파라미터 조정 300개의 Tree와 4개의 변수 적용 모델 생성 500개는 너무 많소
model = randomForest(Species~., data=iris, 
...
# 분류를 하게끔 해주는 공헌도
# 분류정확도 개선에 기여하는 변수
# MeanDecreaseGini 지니계수 : 노드 불순도를 측정 : 클수록 노드 불순도, 불확실성에 기여
...
# Type of random forest: regression # 연속형이다
# Mean of squared residuals: 10.00096 # 오차의 제곱의 평균 # 숫자가 커서 데이터가 표준화 필요
# % Var explained: 88.15 # 모델의 예측력 88%
names(boston_model)
...
# 데이터 표준화가 어렵다면
cor(y_true, y_pred) # 0.9421195 정답과 예측치의 관계의 상관성이 오졌따 -> 예측력 ㅅㅌㅊ인듯
...
model <- randomForest(survived ~., data = titanic_df,  #알아서 랜덤샘플링하니 train test 필요업지
...
#################################################
# MeanDecreaseGini 지니계수 : 노드 불순도를 측정 : 클수록 노드 불순도, 불확실성에 기여
# --> 작을수록 불순도 불확실성에 기여 : 엔트로피
# - tree model에서 중요변수 선정 기준 : 엔트로피
