...
lword <- Map(extractNoun, marketing2)  #Map(function, Data)
length(lword) # [1] 472
lword <- unique(lword) # 중복제거 방법1(전체 대상)
length(lword) # [1] 353(119개 제거)
lword[1] # key[[doc]] : value(word)

lword <- sapply(lword, unique) # 중복제거 방법2(줄 단위 대상) 
length(lword) 
class(lword) # list
lword # 추출 단어 확인
# [[353]] 키 및 문서 번호
...
wordtran 
# 353 transactions (rows) and 문서의 수
# 2423 items (columns) 단어의 수
