...
# score 변수 : A레벨 화학점수(0,2,4,6,8,10) 숫자이나 카테고리임 Not연속
# gender 변수 : 성별
# gcsescore 변수 : 고등학교 재학중에 치루는 큰 시험 like 모의고사
...
table(Chem97$score)
 ...
 histogram(~gcsescore | score, data=Chem97) # x축 : score범주 
 # 점수 높은 애들이 모의고사도 잘쳤더라
 histogram(~gcsescore | factor(score), data=Chem97)
 # score 요인 단위 : 가독성 개선
 # | factor(집단변수) : 집단 수 만큼 격자 생성

# 2.densityplot(~x축(보통 연속) | 집단변수, dataframe, groups=변수)
...
# 3.barchart(y~x | 조건, dataframe, layout)        y축, x축 | 조건, 대상, 격자 와꾸
barchart(Var1 ~ Freq | Var2, data=dft, layout=c(4,1)) # 4행 1열
barchart(Var1 ~ Freq | Var2, data=dft, layout=c(2,2)) # 2행 2열
...
# 차트 상단의 막대 범위의 아래 위로 겹치는 정도 : 오버랩
# row : 패널 행수
# 6numner, 2row : 2행 3열
...
qplot(hwy, data=mpg, fill=drv, binwidth=2) # binwidth 옵션 적용 도수 폭이 넓어짐
...
qplot(wt, mpg, data=mtcars, color=carb) # factor로 지정하지 않으면 연속변수로 인식함
qplot(wt, mpg, data=mtcars, color=factor(carb)) # 색상 적용, 이산변수이기에 이 편이 더 설명력이 좋음
...
# clarity가로 범주 단위, cut 세로 범주 단위, 누적형
...
# 스무쓰가 아래위 라인을 만듬 : 가운데 선이 회귀선, 아래 위 선이 오차범위
qplot(wt, mpg, data=mtcars, geom=c("point", "line")) # 얘랑 비교할 것
...
# 으따 비례관계구먼 근대 심하게 정비례는 아니니 부모키가 그렇게 큰 영향력은 아니구먼
# 스무쓰로 인하여 회귀선 아래위로 진한 회색 영역이 생김 : 오차범위
...
# (2) geom_line() 레이어 추가 
p+geom_line() # line 추가 # 기존의 객체는 바뀌지 않음
...
# 밀도 1기준으로 꽉 찬 막대차트
# clarity와 cut과의 관계 규명

p <- p+geom_point()
ggsave(file="C:/itwill/2_Rwork/output/bar.pdf", plot=p, width=10, height=5)
...
seoul <- c(left = 126.77, bottom = 37.40, 
           right = 127.17, top = 37.70) # 어느 범위까지 따올 것인지 지정해줘야 함
# 구글 통하는 거 쓰면 더 편해짐 
...
# [단계2] 지도정보 생성
map <- get_stamenmap(daegu, zoom=7,  maptype='terrain')#'toner-2011')
ggmap(map)
...
layer2 <- geom_point(data = df, aes(x=lon, y=lat,
...
layer3 <- geom_text(data = df, aes(x = lon+0.01, y = lat+0.08, label = region), size = 3)
