...
shapiro.test(z)
# W = 0.9985, p-value = 0.5563 0.05보다 p가 큼 : 정규분포임
hist(z, freq=T)
...
# 범위가 각자 다 다름
# 큰 값에 휘둘리지 않게끔 이를 통일되게끔 함
# 서로 다른 변수의 값을 일정한 값으로 조정
# nor = (x - min) / (max - min)

nor <- function(x){ #0~1 정규화 해주는 함수
  re = (x - min(x)) / (max(x) - min(x))
  return(re)
} # 일명 민맥스 함수

summary(iris[-5])
# 요약통계 선확인 : 각자 민맥스, 범위가 다름

nor_re <- apply(iris[-5], 2, nor)
nor_re
summary(nor_re) # 모든 변수들의 민맥스가 정규화됨


#############################################
# 추론통계분석 - 1-1. 단일집단 비율차이 검정
#############################################
# - 단일 집단(new집단)의 비율이 어떤 특정한 값(old집단)과 같은지를 검증
...
# alternative="two.sided", conf.level=0.95) 얘내들은 기본값임
# p-value = 0.0006735 # < 0.05 이므로 귀무가설 기각

# 방향성이 있는 대립가설 단측 검정 # new > old 인가?
binom.test(14, 150, p=0.2, alternative="greater", conf.level=0.95)
# p-value = 0.9999 # 기각

# [실습]방향성이 있는 대립가설 검정 # new < old 인가?
...
shapiro.test(x1) # 정규분포 검정 함수(p-value = 0.7242) # > 0.05 정규분포와 차이가 없다
...
# t = 3.9461, df = 108
# t 검정통계량 채택역 : -1.96 ~+1.96 # t z f 도 마찬가지
# p-value = 0.0001417
# 채택역 밖에 있는 셈이다 이렇게도 해석된다
...
# p-value = 0.0001711 코딩교육 선호도가 유의미하게 높다
...
# 3. 분포모양 검정 
var.test(before, after, paired=TRUE) #paired = T : before-after 관계가 맞다
# 0.05보다 크니 모수검정
# 동질성 분포 : t.test()
# 비동질성 분포 : wilcox.test()

# 4. 가설검정
t.test(before, after, paired=TRUE) # p-value < 2.2e-16 귀무가설을 기각한다
...
boxplot(data$score)$stats # 이를 확인하고 없애야 할 극단치를 족침
...
# F value가 중요 저 값이 -1.96~1.96 사이의 값이 나온다면 채택할만 한 거임
# 저 값은 채택력을 많이 벗어낫고
# 이에 근거하여 채택할 가능성이 연산되어 Pr(>F) 이게 오지게 작게 나옴 (서로 반비례관계)
# 귀무가설 : 집단 간 평균에 차이가 없다.
# Residuals : 오차 및 잔차
# method2의 ***표시 : 매우 유의미한 수준에서 적어도 한 집단에서 평균에 차이를 보인다

# 그래서 사후검정을 해야 함 : 터키 함수
TukeyHSD(result) # 항상 95% 신뢰수준
#               diff        lwr        upr     p adj
...
# 방법2-방법1 차이(diff)가 젤 심하다 : 2.61만큼
# lwr : 95% 신뢰수준 하한값, upr : 상한값
# p adj : 쟤내들 다 0.05보다 작은데, 유의미한 수준에서 집단에서 차이를 보인다는 거

# 따라서 세 집단 모두 집단의 차이를 보인다.

plot(TukeyHSD(result))
# 시각화로 마무리
# 만약 표시되는 선이 0과 겹쳐지면 차이가 없는 걸로 이해하면 댐
...
############################################
# 종속변수 ~ 독립변수1 + 독립변수2
# - 쇼핑몰 고객의 연령대별(20~50대), 시간대별(오전/오후) 구매현황 분석
# - 독립변수 : 연령대, 시간대
# - 종속변수 : 구매현황
...
data <- data.frame(age, time, buy) # 독립 독립 종속

# 연속형 -> 범주형
data$age2 [data$age <= 29] <- 20
data$age2 [data$age > 29 & data$age <= 39] <- 30
data$age2 [data$age > 39 & data$age <= 49] <- 40
...
# 0.05보다 커서 데이터가 동질하다 판단

# 3. 분산분석
# 귀무가설 : 집단간 평균에 차이가 없다
# 대립가설 : 적어도 한 집단엔 평균에 차이가 있다.

resule <- aov(formula = buy ~ age2 + time, data = data)
summary(resule)
# f value, pvalue를 보아하니 채택역에 충분히 들어간다
# 평균의 차이가 없다
