...
sqrt(sum((x[1,] - x[2,])^2)) #dist[1,2] 5.196152
sqrt(sum((x[1,] - x[3,])^2)) #dist[1,3] 10.3923
...
# 군집의 수는 관찰자가 결정함 but 군집의 수를 미리 알고 분석하는 것이 아님
# 모르니까 일단 이렇게 돌려보고 몇 군집으로 해야겠다는 판단의 근거를 만드는 것임
# 3개 그룹 선정, 선 색 지정
rect.hclust(hc, k=3, border="red") # 3개 그룹 선정, 선 색 지정

# 각 그룹별 서브셋 만들기 # 저 네모칸 별로 나눴음
...
# 1) 유클리드 거리 계산 
idist<- dist(iris[1:4]) # dist(iris[, -5]) # 거리계산이니 숫자연산만 가능하므로

# 2) 계층형 군집분석(클러스터링)
hc <- hclust(idist)
hc
# Cluster method   : complete / (기본값) 가장 먼 거리를 기준으로 가까운 정도를 판단,
                                #        평균으로 하려면 average 등 다른 지정 가능함

plot(hc, hang=-1)
rect.hclust(hc, k=3, border="red") # 3개 그룹수 

# 자르긴 했는데 어떤 것을 의미하게끔 잘렸는가
...
table(result2$cluster) # 102 291 607
result2$centers # 각 변수에 대한 군집의 중앙값
...
# 가장 높은 상관관계 두 놈을 데려옴 : 군집을 이해하는 데에 가장 효과적임
# 6) 비계층적 군집시각화
...
# 근대 내가 군집 수를 잘 결정하긴 하였는가? 점검이 필요하다
###################################################
# 군집수 결정방법
###################################################
install.packages("NbClust") # 넘버클러스트
library(NbClust)
# 아이리스 최적 군집수는??
data(iris)
iris_max <- as.matrix(iris[-5]) # 150 * 4

?NbClust
# min.nc = 2 최소 군집 몇개부터 시작할까, max.nc 최대 몇개까지?
# method는 딱 지정해줘야 함
