...
-- 조건2) 고객ID 오름차순 정렬
select u.user_id, u.gender, u.age, job, p.product_type, p.pay_method, p.price
from user_data u, pay_data p
where u.user_id = p.user_id order by u.user_id;
-- [해결] 전체 고객 중에서 상품 구매 고객 정보만 조회

-- 문2) 문1)의 결과에서 성별이 '여자'이거나 지불방법이 '1.현금'인 경우만 출력하시오. gender=1 : 남자. 2: 여자
select u.user_id, u.gender, u.age, job, p.product_type, p.pay_method, p.price
from user_data u, pay_data p
where u.user_id = p.user_id and (u.gender = 2 or p.pay_method = '1.현금')
-- [해설] 상품 구매 고객 중에서 특정 고객 조회
...
-- [해설] 전체 고객 중에서 상품을 구매하지 않은 고객까지 조회
